...
class MyDB:
	...
	def __init__(self,host,root,pwd,db,chset):
		try:
			self.conn = MySQLdb.connect(host = 'localhost', user = 'root', passwd = '', db = 'novel', charset = 'utf8')
	...
	# save chapter 
	def saveChapter(self,book_id,chapter_title,chapter_url):
		try:
			value = [book_id,chapter_title,chapter_url]
			# sql = 'INSERT INTO mo_tag (kind_id, tag_name, tag_url) VALUES (%s, %s, %s)'
			sql = 'INSERT INTO novel_chapter (book_id,chapter_title,chapter_url) VALUES (%s, %s, %s)'
	...
	# save content
	def saveContent(self,chapter_id,content_text):
		try:
			value = [chapter_id,content_text]
			# sql = 'INSERT INTO mo_tag (kind_id, tag_name, tag_url) VALUES (%s, %s, %s)'
			sql = 'INSERT INTO novel_content (chapter_id,content_text) VALUES (%s, %s)'
			self.cur.execute(sql,value)
			self.conn.commit()
			return True
		except MySQLdb.Error,e:
